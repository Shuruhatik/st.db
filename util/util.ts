import { access, writeFileSync, constants, statSync } from "node:fs";
...
const createFile = (path: string, stringify: (x: any) => any, storageStyle: storageStyle): void => {
    writeFileSync(path, stringify(storageStyle.writing_format(new Map())))
};
...
export {
    createFile, pathResolve, file_exists, YAML_parse, JSON_stringify, YAML, formatErrorMessage
};