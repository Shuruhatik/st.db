...
export class Collection {
    ...
    * @returns {void}
    */
    math(key: string, operator: "-" | "+" | "*" | "/", value: string | number, goToNegative: boolean = false): void {
        if (!/^-?\d+$/.test(`${value}`)) throw Error(formatErrorMessage("Invlid Type Value", 'Collection', 'Method', "math"))
        if (!`${operator}`.match(/(\-|\+|\/|\*)/g)) throw Error(formatErrorMessage("Invlid Type Operator", 'Collection', 'Method', "math"))
        if (typeof goToNegative != "boolean") throw Error(formatErrorMessage("Invlid Type goToNegative", 'Collection', 'Method', "math"))
        if (!this.has(key)) {
            this.set(key, value)
        } else {
    ...
    delete(key: any): void {
        if (key instanceof Object) {
            ...
            this.map.forEach((value, item) => {
                count++
                if (JSON.stringify(key) != JSON.stringify(item)) new_collection.set(value, item)
