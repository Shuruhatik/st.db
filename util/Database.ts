...
class Database extends EventEmitter {
    ...
    constructor(path: string, options: DatabaseOptions = {}) {
        ...
        this.crypto = new Crypto({ digest: options.encryption?.digest || "sha512" });
    ...
    */
    async writeFileSync(collection: Collection): Promise<void> {
        await writeFileSync(this.path, this.stringify(this.storageStyle.writing_format(collection.map)))
    }
    /**
    ...
    */
    ...
    shift(key: any): Promise<ElementInData> {
        if (key.key) key = key.key;
        if (!key && key != 0) throw Error(formatErrorMessage("No key found", 'Database', 'Method', "shift"))
        if (this.has(key) == false) return this.set(key, []);
        else {
            let value = this.get(key)
            if (Array.isArray(value)) { value.shift(); return this.set(key, value) }
    ...
    pop(key: any): any {
        ...
        if (this.has(key) == false) return this.set(key, []);
            ...
            let value = this.get(key)
            if (Array.isArray(value)) { value.pop(); return this.set(key, value) }
            else return this.set(key, []);
        }
    }
    /**
    ...
    */
    async pull(key: any, value?: any): Promise<ElementInData> {
        if (key.value) value = key.value; if (key.key) key = key.key;
        if (!value && value != 0 || !key && key != 0) throw Error(formatErrorMessage("No key or value found", 'Database', 'Method', "pull"))
        if (this.has({ key }) == false) throw Error(formatErrorMessage("There is no data to execute on it", 'Database', 'Method', "pull"))
        return this.set(key, this.get({ key }).filter((v: any) => v !== value));
    }
    /**
    ...
    */
    ...
    type(key: any): ("symbol" | "array" | "undefined" | "string" | "number" | "bigint" | "boolean" | "object" | "function") {
        if (key.key) key = key.key;
        if (!key && key != 0) throw Error(formatErrorMessage("No key found", 'Database', 'Method', "type"))
    ...
    */
    startsWith(key: any): { ID: any, typeof: { ID: string, data: string }, data: any }[] {
        return key.key ? this.filter((element: any) => element.ID.startsWith(key.key)) : this.filter((element: any) => element.ID.startsWith(key));
    }
    /**
    ...
    * @returns {Promise<{ ID: any, typeof: { ID: string, data: string }, data: any }[]>}
    */
    endsWith(key: any): { ID: any, typeof: { ID: string, data: string }, data: any }[] {
        return key.key ? this.filter((element: any) => element.ID.endsWith(key.key)) : this.filter((element: any) => element.ID.endsWith(key));
    }
    /**
     ...
     * @returns {Promise<{ ID: any, typeof: { ID: string, data: string }, data: any }[]>}
     */
    filter(callback: any, argument?: any): { ID: any, typeof: { ID: string, data: string }, data: any }[] {
    ...
    * @returns {Promise<any[]>}
    */
    includes(key: any): { ID: any, typeof: { ID: string, data: string }, data: any }[] {
        return key.key ? this.filter((element: any) => element.ID.includes(key.key)) : this.filter((element: any) => element.ID.includes(key));
    }
    /**
    ...
    * @return {Promise<void>}
    */
    async math(key: { key: any, value: any, goToNegative?: boolean, operator: ("-" | "+" | "*" | "/") } | any, operator?: ("-" | "+" | "*" | "/"), new_value?: (string | number), goToNegative?: boolean): Promise<void> {
        if (!key && key != 0) throw Error(formatErrorMessage("No key found", 'Database', 'Method', "math"))
    ...
    async add(key: ({ key: any, value: any } | any), value?: any): Promise<void> {
        if (key.value) value = key.value; if (key.key) key = key.key
        if (!value && value != 0 || !key && key != 0) throw Error(formatErrorMessage("No key or value found", 'Database', 'Method', "add"))
     ...
     * @returns {Promise<void>}
     */
    async multiply(key: ({ key: any, value: any } | any), value?: any): Promise<void> {
        if (key.value) value = key.value; if (key.key) key = key.key
     ...
     * @returns {Promise<void>}
     */
    async double(key: ({ key: any, value: any } | any), value?: any): Promise<void> {
        if (key.value) value = key.value; if (key.key) key = key.key
        if (!value && value != 0 || !key && key != 0) throw Error(formatErrorMessage("No key or value found", 'Database', 'Method', "double"))
     ...
     * @returns {Promise<void>}
     */
    async subtract(key: ({ key: any, value: any } | any), value?: any): Promise<void> {
        if (key.value) value = key.value; if (key.key) key = key.key
        if (!value && value != 0 || !key && key != 0) throw Error(formatErrorMessage("No key or value found", 'Database', 'Method', "subtract"))
        return await this.math(key, "-", value);
    }
    /**
